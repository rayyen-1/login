...
void statusPengirim() {
    ...
    do
    {
        ...
        case 1:
        {
            ...
            // Ambil hanya pengiriman yang ditugaskan ke kurir yang sedang login
        ...
        case 2:
        {
            ...
            // Ambil hanya pengiriman yang ditugaskan ke kurir yang sedang login,
            // supaya seorang kurir tidak bisa melihat/mengubah status pengiriman kurir lain
            vector<int> indeksMilikSaya;
            for (size_t i = 0; i < listKurir.size(); i++) {
                if (listKurir[i].kurir.username == userAktif->username) {
                    indeksMilikSaya.push_back((int)i);
            ...
            // Tampilkan hanya daftar pengiriman milik kurir aktif
            for (size_t n = 0; n < indeksMilikSaya.size(); n++) {
                int i = indeksMilikSaya[n];
                cout << "|| "
                    << left << setw(colNo) << n + 1 << " | "
            ...
            if (pilihan > 0 && pilihan <= (int)indeksMilikSaya.size()) {
                int idxAsli = indeksMilikSaya[pilihan - 1]; // index sebenarnya di listKurir

                if (listKurir[idxAsli].status == sedang_proses_packing) {
                    // Paket masih diproses/dipacking oleh mitra. Kurir belum boleh
                    // mengubah statusnya sampai mitra menyerahkan paket ke kurir
                    // (yang otomatis mengubah status ini jadi "Dalam Perjalanan").
                    ...
                    // Status == dalam_perjalanan, satu-satunya transisi valid untuk kurir
                    // sekarang adalah menandai pengiriman sebagai selesai.
                    char konfirmasi;
        ...
        if (listKurir[i].kurir.username == userAktif->username) {
            ...
            for (int j = 0; j < listKurir[i].beli.jumlahBarang; j++) {
                string baris = "    - " + listKurir[i].beli.daftarBarang[j].namaProduk
                    + " (" + to_string(listKurir[i].beli.daftarBarang[j].banyakPembelian) + " pcs)";
